...
class Scoreboard:
    ...
    def check_high_score(self):
        ...
        if self.stats.score > self.stats.high_score:
            ...
            self.prep_high_score()
            self.stats.save_high_score()   # ← persist it immediately

    def prep_ships(self):
        """Show how many ships are left."""
        self.hearts = pygame.sprite.Group()

        heart_image = pygame.image.load('images/heart.bmp')
        heart_image = pygame.transform.smoothscale(heart_image, (30, 30))


        for heart_number in range(self.stats.hearts_left):
            heart_rect = heart_image.get_rect()
            heart_rect.x = 10 + heart_number * (heart_rect.width + 5)
            heart_rect.y = 10



            heart_surface = heart_image.copy()
            heart_sprite = pygame.sprite.Sprite()
            heart_sprite.image = heart_surface
            heart_sprite.rect = heart_rect
